import telebot, sqlite3, os, threading
...
def apply_referral(new_user_id, ref_id):
    ...
    if cursor.fetchone():
        cursor.execute("UPDATE users SET ref = ref + 1, points = points + 5 WHERE id = ?", (ref_id,))
...
@bot.message_handler(func=lambda msg: msg.text.lower() in VIDEO_CODES)
